...
URL="https://api.openweathermap.org/data/2.5/weather?q=Rockville,MD,US&appid=$APIKEY"

function getData {
    ERROR=0
    # For logging purposes
     #echo " " >> "$HOME/.weather.log"
     #echo $APIKEY >> "$HOME/.weather.log"
...
if [ $ERROR -eq 0 ]; then
    ...
    DATE=`date +%s`
    WIND=""
    HOLDING=$(echo "$RESPONSE" | jq -r .main.temp)
    TEMP=$(jq -r '.main.temp | (. - 273.15) * 9/5 + 32' <<< "$RESPONSE")
